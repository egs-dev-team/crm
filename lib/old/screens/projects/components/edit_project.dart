...
import 'package:egs/old/screens/projects/components/journal.dart';
...
class AddEditProjectScreenState extends State<AddEditProjectScreen> {
  ...
  void initState() {
    super.initState();
    _projTypeController =
        TextEditingController(text: widget.initialProject?.proj_type ?? '1');
    _nameController =
        TextEditingController(text: widget.initialProject?.name ?? '');
    _regNumController =
        TextEditingController(text: widget.initialProject?.reg_num);
    _contractController =
        TextEditingController(text: widget.initialProject?.contract);
    _dateCreationController = TextEditingController(
        text: widget.initialProject?.date_creation?.toLocal().toString().substring(0, 10));
    _dateNotificationController = TextEditingController(
        text: widget.initialProject?.date_notification?.toLocal().toString().substring(0, 10));
    _objectTypeController =
        TextEditingController(text: widget.initialProject?.object_type);
    _addressController =
  ...
  void _saveOrUpdateProject() async {
    try {
      ...
      if (widget.initialProject == null) {
        ...
        var createdProject = await apiService.createProject(project);
        setState(() {});
        ...
        var updatedProject = await apiService.updateProject(myId, project);
