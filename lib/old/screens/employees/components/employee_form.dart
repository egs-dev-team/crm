...
class _EmployeeFormState extends State<EmployeeForm> {
  ...
  void initState() {
    ...
    birthController = TextEditingController(
        text:
            widget.user?.date_of_birth?.toLocal().toString().substring(0, 10));
    dateofstartController = TextEditingController(
        text:
            widget.user?.date_of_start?.toLocal().toString().substring(0, 10));
    ...
    infoaboutrelocateController =
        TextEditingController(text: widget.user?.info_about_relocate ?? '');
  ...
  void dispose() {
    ...
    infoaboutrelocateController.dispose();
    attestationController.dispose();
    qualificationController.dispose();
    retrainingController.dispose();
    super.dispose();
  }
  ...
  Future<void> fetchInitialData() async {
    try {
      if (widget.user != null) {
        setState(() async {
          tasksByDate = apiService.fetchTasksByDate(widget.user!.id!);
        });
      } else {
  ...
  void saveUser() async {
    try {
      ...
      final User newUser = User(
        ...
        info_about_relocate: infoaboutrelocateController.text,
      ...
      if (widget.user == null) {
        ...
        if (widget.user?.id != null) {
          ...
          final response =
              await apiService.updateUser(userId: myId, user: newUser);
