...
class FileInfoCard extends StatelessWidget {
  ...
  Color getColor(Task info) {
    return (info.completion!.difference(DateTime.now()) <
                    const Duration(days: 7) &&
                DateTime.now().isBefore(info.completion!)) ||
            DateTime.now().isAfter(info.completion!)
        ? TaskStatusColor.textEmergency
        : ((info.completion!.difference(DateTime.now()) <
                    const Duration(days: 14) &&
                DateTime.now().isBefore(info.completion!))
            ? TaskStatusColor.textSeason
            : TaskStatusColor.textInWork);
  }
}
